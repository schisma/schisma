...
testBabo =
  ...
  in  testGroup
        "babo"
        [ testCase "babo opcode generation" $ baboBlock @?= sampleBaboOutput
        , testCase "baboWithDefaults opcode generation"
        $   baboWithDefaultsBlock
        @?= sampleBaboWithDefaultsOutput
        ]
...
sampleBaboWithDefaults = baboWithDefaults input
                                          (x    , y    , z)
                                          (width, depth, height)
 where
  input  = oscilWithDefaults (a# 0.5) (a# 500.0)
...
sampleBaboWithDefaultsOutput :: [Text]
sampleBaboWithDefaultsOutput =
  [ "instr 1"
  , "a1 = 0.5"
  , "a2 = 500.0"
